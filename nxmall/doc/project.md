...
```
    ...
    <logger name="org.mallmark.litemall.core" level="DEBUG" />
    <logger name="org.mallmark.litemall.db" level="DEBUG" />
    <logger name="org.mallmark.litemall.admin" level="DEBUG" />
    <logger name="org.mallmark.litemall.wx" level="DEBUG" />
    <logger name="org.mallmark.litemall" level="DEBUG" />
```

具体如何配置，请自行学习Spring Boot的日志配置和logback日志配置。

`org.mallmark.litemall.core`定义litemall-core模块的日志级别
`org.mallmark.litemall.db`定义litemall-db模块的日志级别
`org.mallmark.litemall.wx`定义litemall-wx-api模块的日志级别
`org.mallmark.litemall.admin`定义litemall-admin-api模块的日志级别
`org.mallmark.litemall`而定义litemall所有后端模块的日志级别
